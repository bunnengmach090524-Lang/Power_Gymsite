...
const form = useForm({
  trainer_id: props.gymClass.trainer_id ?? '',
  name: props.gymClass.name,
  description: props.gymClass.description ?? '',
  schedule_day: props.gymClass.schedule_day,
  start_time: props.gymClass.start_time?.slice(0, 5) ?? '',
  end_time: props.gymClass.end_time?.slice(0, 5) ?? '',
  capacity: props.gymClass.capacity,
  price: props.gymClass.price,
  image: null,
})

const imagePreview = ref(props.gymClass.image_url ?? null)
function onImageChange(e) {
  const file = e.target.files[0]
  form.image = file ?? null
  imagePreview.value = file ? URL.createObjectURL(file) : props.gymClass.image_url ?? null
}

function submit() {
  // Laravel doesn't read multipart bodies on PUT, so this goes as POST
  // with an explicit _method override — same pattern the backend already
  // expects from ClassController::update's hasFile() check.
  form.transform(data => ({ ...data, _method: 'put' })).post(`/dashboard/classes/${props.gymClass.id}`, {
    forceFormData: true,
  })
}
</script>
...
<template>
  <div class="w-full animate-fade-in-up">
    ...
    <h1 class="text-2xl font-semibold text-slate-900 dark:text-white mb-6">{{ t.class_edit_title }}</h1>
    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
        ...
        <!-- FIX: was missing entirely — admins had no way to change price after creation -->
        <div>
          <label class="block text-base text-slate-700 dark:text-slate-300 mb-2">
            {{ t.class_price ?? 'តម្លៃ ($)' }}
      ...
      <div class="flex flex-col sm:flex-row gap-3 mt-6">
        ...
        >
          {{ form.processing ? t.class_saving : t.class_save_changes }}
        </button>
        <Link
          href="/dashboard/classes"
