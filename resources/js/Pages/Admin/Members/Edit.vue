...
function submit() {
  form.put(`/dashboard/members/${props.member.id}`)
}
</script>
...
<template>
  <div class="w-full animate-fade-in-up">
    ...
    <h1 class="text-2xl font-semibold text-slate-900 dark:text-white mb-6">{{ t.member_edit_title }}</h1>
    ...
    >
      ...
      <div class="flex flex-col sm:flex-row gap-3 mt-6">
        ...
        >
          {{ form.processing ? t.member_saving : t.member_save_changes }}
        </button>
        <Link
          href="/dashboard/members"
          class="flex-1 text-center border border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-base rounded-lg py-3 transition-all duration-200 hover:bg-slate-50 dark:hover:bg-slate-800"
        >
          {{ t.member_cancel }}
        </Link>
      </div>
