...
const props = defineProps({
  ...
  // Passed from Promotions/Index.vue's "+ Add Discount" quick-action
  // (?plan_id=X on the create route) so the plan dropdown is pre-filled
  // instead of the admin having to find it again in a long list.
...
const preselectedPlan = props.preselectedPlanId
  ? props.membershipPlans.find(p => p.id === props.preselectedPlanId)
  : null
...
const form = useForm({
  ...
  discount_value: '',
  applicable_plan_id: props.preselectedPlanId ?? '',
  start_date: '',
  end_date: '',
  active: true,
})
...
function submit() {
  form.post('/dashboard/promotions')
}
</script>
...
<template>
  <div class="w-full animate-fade-in-up">
    ...
    <h1 class="text-2xl font-semibold text-slate-900 dark:text-white mb-2">{{ t.promo_add_title }}</h1>

    <!-- Context banner when arriving via "+ Add Discount" on a specific plan -->
    <div
      v-if="preselectedPlan"
      class="inline-flex items-center gap-2 text-sm bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-lg px-3.5 py-2 mb-4"
    >
      🔥 {{ lang === 'km' ? `កំពុងបង្កើត Discount សម្រាប់ "${preselectedPlan.name}"` : `Creating a discount for "${preselectedPlan.name}"` }}
    </div>
    ...
    >
      ...
      <button
        ...
        class="mt-6 w-full md:w-auto md:px-10 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 disabled:hover:bg-emerald-500 text-slate-950 font-medium text-base rounded-lg py-3 transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/20 hover:scale-[1.02] active:scale-[0.98]"
      >
        {{ form.processing ? t.promo_saving : t.promo_save }}
      </button>
    </form>
  </div>
</template>
