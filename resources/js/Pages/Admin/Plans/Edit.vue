...
// NOTE: prop is "plan", not "class" — safe here since "plan" isn't a
// reserved Vue/HTML attribute name (unlike the GymClass "class" prop bug
// fixed earlier). No renaming needed.
...
function submit() {
  form.put(`/dashboard/plans/${props.plan.id}`)
}
</script>
    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
        ...
        <div>
          ...
          <input v-model.number="form.price" type="number" min="0" step="0.01" :class="fieldClass" />
      ...
      <div class="flex flex-col sm:flex-row gap-3 mt-6">
        ...
        >
          {{ form.processing ? (lang === 'km' ? 'កំពុងរក្សាទុក...' : 'Saving...') : (lang === 'km' ? 'រក្សាទុកការផ្លាស់ប្តូរ' : 'Save Changes') }}
        </button>
        <Link
          href="/dashboard/plans"
