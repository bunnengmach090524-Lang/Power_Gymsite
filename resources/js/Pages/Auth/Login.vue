...
<template>
  <div class="min-h-screen bg-slate-950 flex">
    ...
    <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden">
      <!-- swap this div's background for a real gym action photo, e.g.
           bg-[url('/images/gym-hero.jpg')] bg-cover bg-center -->
      ...
      <div class="relative z-10 flex flex-col justify-between p-12 w-full">
        <div class="flex items-center gap-2.5">
          ...
          <span class="text-white font-bold text-lg">GymSite</span>
        </div>

        <div>
          <h2 class="text-4xl font-extrabold text-white leading-tight mb-4">
            គ្រប់គ្រង Gym<br />របស់អ្នកឲ្យបានងាយស្រួល
          </h2>
          <p class="text-slate-300 text-base max-w-sm mb-8">
            ចូលគណនីដើម្បីគ្រប់គ្រង member, class schedule, និង payment ទាំងអស់ក្នុងកន្លែងតែមួយ
          </p>
          <div class="space-y-3">
            <div class="flex items-center gap-3 text-slate-200 text-sm">
              ...
              គ្រប់គ្រង member និង membership
            </div>
            <div class="flex items-center gap-3 text-slate-200 text-sm">
              <span class="w-6 h-6 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0">
                <svg class="w-3.5 h-3.5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
              </span>
              តាមដាន class booking និង payment
            </div>
            <div class="flex items-center gap-3 text-slate-200 text-sm">
              <span class="w-6 h-6 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0">
                <svg class="w-3.5 h-3.5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
              </span>
              សុវត្ថិភាព និងគួរឲ្យទុកចិត្ត
            </div>
          </div>
        </div>

        <p class="text-slate-500 text-xs">&copy; {{ new Date().getFullYear() }} GymSite. All rights reserved.</p>
      </div>
    </div>
    ...
    <div class="flex-1 flex items-center justify-center px-4 py-12">
      <div class="w-full max-w-md">
        <!-- mobile-only brand (visual panel is hidden below lg) -->
        <div class="text-center mb-8 lg:hidden">
          <h1 class="text-2xl font-bold text-white">GymSite</h1>
        </div>

        <div class="mb-8">
          <h2 class="text-2xl font-bold text-white mb-1.5">សូមស្វាគមន៍មកវិញ 👋</h2>
          <p class="text-sm text-slate-400">ចូលគណនីដើម្បីបន្តគ្រប់គ្រង Gym របស់អ្នក</p>
        </div>

        <!--
          FIX: Google OAuth failures (e.g. unrecognized account) come back as a
          flash session error via withErrors(['email' => ...]) in
          GoogleAuthController@callback. That lands in page.props.errors, NOT
          in this local useForm()'s form.errors (which only fills after a
          POST /login submit). Without this block the user saw nothing at all
          when Google login failed — a silent redirect back to this page.
        -->
        <p
        ...
        <p class="text-center text-sm text-slate-400 mt-6">
          មិនទាន់មានគណនី?
          <Link href="/register" class="text-emerald-400 font-medium hover:text-emerald-300">បង្កើតគណនីថ្មី</Link>
        </p>
      </div>
    </div>
  </div>
