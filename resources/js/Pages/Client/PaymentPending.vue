...
async function checkStatus() {
  ...
  try {
    const res = await fetch(`/gym/${props.tenant.slug}/payment-requests/${props.paymentRequestId}/status`, {
...
function markAsPaidManually() {
  router.visit(`/gym/${props.tenant.slug}/account`, {
    data: { note: 'Payment submitted, awaiting admin confirmation' }
  })
}
...
function shareText() {
  return `ទូទាត់ចំនួន $${props.plan.price} សម្រាប់ ${props.tenant?.name} — ${window.location.href}`
}

function shareToTelegram() {
  const url = encodeURIComponent(window.location.href)
  const text = encodeURIComponent(`ទូទាត់ចំនួន $${props.plan.price} សម្រាប់ ${props.tenant?.name}`)
  ...
  <div class="relative min-h-screen w-full overflow-hidden bg-slate-900">
    ...
    <div class="relative z-10 min-h-screen flex items-center justify-center px-4 py-20">
      <div class="w-full max-w-md bg-white/95 backdrop-blur-xl rounded-3xl p-6 sm:p-8 text-center shadow-2xl border border-white/60">
        ...
        <h1 class="text-lg font-semibold text-slate-900 mb-5">{{ plan.name }}</h1>
        ...
        <div class="flex items-center justify-center gap-2 mb-6">
          <div class="flex items-center gap-1.5">
            ...
            <span class="text-xs text-slate-500 hidden sm:inline">ជ្រើសរើសគម្រោង</span>
          </div>
        ...
        <p class="text-3xl font-extrabold text-emerald-500 mb-6">${{ plan.price }}</p>
        ...
        <div v-if="status === 'pending'">
          ...
          <p v-if="!isExpired" class="text-sm text-slate-500 mb-1">ស្កេន QR ដើម្បីទូទាត់ជាមួយ Bakong / ធនាគារណាមួយ</p>
          ...
          <button
            v-else
            @click="markAsPaidManually"
            class="mt-4 w-full bg-emerald-500 hover:bg-emerald-400 text-white text-sm font-medium py-2.5 rounded-lg transition-colors"
          >
            ខ្ញុំបានទូទាត់រួច
          </button>
        </div>
        ...
        <div v-else-if="status === 'verified'" class="text-emerald-600">
          ...
          <p class="text-lg font-medium">ទូទាត់ជោគជ័យ!</p>
          <p class="text-sm text-slate-500 mt-1">កំពុងបញ្ជូនអ្នកទៅគណនី...</p>
        </div>
      </div>
    </div>
  </div>
