...
function submit() {
  form.post(`/gym/${props.tenant.slug}/register`)
}
...
<template>
  <div class="min-h-screen bg-slate-950 flex">
    ...
    <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden">
      <!-- swap for tenant.cover_image if the gym has uploaded one -->
      <div class="absolute inset-0 bg-gradient-to-br from-emerald-900 via-slate-900 to-slate-950"></div>
      <div class="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(16,185,129,0.15),transparent_50%)]"></div>

      <div class="relative z-10 flex flex-col justify-between p-12 w-full">
        <div class="flex items-center gap-2.5">
          <div v-if="tenant.logo_url" class="w-10 h-10 rounded-xl overflow-hidden">
        ...
        <div>
          <h2 class="text-4xl font-extrabold text-white leading-tight mb-4">
            ចូលរួមជាមួយ {{ tenant.name }}<br />ថ្ងៃនេះ
          </h2>
          <p class="text-slate-300 text-base max-w-sm mb-8">
            ចុះឈ្មោះជាសមាជិកដើម្បីកក់ class, តាមដានការហាត់ប្រាណ និងទទួល offer ពិសេស
          </p>
          <div class="space-y-3">
            ...
            <div class="flex items-center gap-3 text-slate-200 text-sm">
              ...
              ជជែកជាមួយ Trainer ផ្ទាល់
            </div>
          </div>
        </div>

        <p class="text-slate-500 text-xs">&copy; {{ new Date().getFullYear() }} {{ tenant.name }}</p>
      </div>
    </div>
    ...
    <div class="flex-1 flex items-center justify-center px-4 py-12">
      <div class="w-full max-w-md">
        ...
        <div class="mb-8">
          <h2 class="text-2xl font-bold text-white mb-1.5">ចុះឈ្មោះជាសមាជិក</h2>
          <p class="text-sm text-slate-400">ចូលរួមហើយចាប់ផ្តើមតាមដានការហាត់ប្រាណរបស់អ្នក</p>
        </div>
        ...
        <div class="flex items-center gap-3 mb-5">
          ...
          <span class="text-xs text-slate-500">ឬ</span>
          <div class="flex-1 h-px bg-slate-800"></div>
        </div>

        <form @submit.prevent="submit" class="space-y-4">
          <div>
            <label class="block text-sm text-slate-300 mb-1.5">ឈ្មោះពេញ</label>
            <div class="relative">
              ...
              <input v-model="form.name" type="text" class="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-3.5 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition" />
            </div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input v-model="form.email" type="email" class="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-3.5 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition" />
            </div>
            <p v-if="form.errors.email" class="text-red-400 text-xs mt-1.5">{{ form.errors.email }}</p>
          </div>

          <div>
            <label class="block text-sm text-slate-300 mb-1.5">លេខទូរស័ព្ទ (មិនចាំបាច់)</label>
            <div class="relative">
              <svg class="w-4.5 h-4.5 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
              ...
              <input v-model="form.phone" type="text" class="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-3.5 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition" />
            </div>
          </div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input v-model="form.password" :type="showPassword ? 'text' : 'password'" class="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-10 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition" />
          ...
          <div>
            <label class="block text-sm text-slate-300 mb-1.5">បញ្ជាក់ពាក្យសម្ងាត់</label>
            <div class="relative">
              <svg class="w-4.5 h-4.5 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
              </svg>
              <input v-model="form.password_confirmation" :type="showConfirm ? 'text' : 'password'" class="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-10 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition" />
          ...
          <button
            ...
            class="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 disabled:cursor-not-allowed text-slate-950 font-semibold rounded-lg py-3 transition mt-2"
          >
            {{ form.processing ? 'កំពុងចុះឈ្មោះ...' : 'ចុះឈ្មោះ' }}
        ...
        <p class="text-center text-sm text-slate-400 mt-6">
          មានគណនីរួចហើយ?
          <Link :href="`/gym/${tenant.slug}/login`" class="text-emerald-400 font-medium hover:text-emerald-300">ចូលគណនី</Link>
        </p>
      </div>
    </div>
  </div>
